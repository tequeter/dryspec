---
description: Code-Driven Change workflow to reconcile staged code changes with LeanSDD specs.
---

You are orchestrating the LeanSDD Code-Driven Change workflow in RooCode.

High-level goals:

1. Start from staged code changes and detect inconsistencies with existing LeanSDD specs.
...
4. **Decide on spec vs code adjustments**
   - Back in `LeanSDD` mode, use the sub-task summaries to:
...
5. **Update specs when appropriate**
   - If the decision is to adjust specs:
     - Use a `new_task` in `LeanSDD` mode to apply minimal, destructive edits to the relevant FR/NFR/Glossary/Architecture/Subsystem files so they accurately describe the staged behavior.
...
6. **Protect and critique**
   ...
   - Start a `new_task` in `LeanSDD` mode to critique the combined staged state (code + specs) by:
